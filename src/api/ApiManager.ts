...
import { IVolxitDefinition } from '../models/IVolxitDefinition'
...
export default class ApiManager {
    ...
    uploadVolxitDefinitionContent(
        appName: string,
        volxitDefinition: IVolxitDefinition,
        gitHash: string,
        detached: boolean
    ) {
        const http = this.http
            ...
            .then(
                http.fetch(
                    ...
                    '/user/apps/appData/' +
                        ...
                        (detached ? '?detached=1' : ''),
                    {
                        volxitDefinitionContent: JSON.stringify(
                            volxitDefinition
                        ),
                        gitHash
                    }
    ...
    setNginxConfig(customBase: string, customVolxit: string) {
        const http = this.http
        ...
        return Promise.resolve() //
            .then(
                http.fetch(http.POST, '/user/system/nginxconfig', {
                    ...
                    volxitConfig: { customValue: customVolxit }
                })
            )
    }
