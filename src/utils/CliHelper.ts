...
export default class CliHelper {
    ...
    findDefaultVolxitName() {
        let currentSuffix = 1
        const machines = StorageHelper.get()
            .getMachines()
            .map(machine => machine.name)
        while (machines.includes(this.getVolxitFullName(currentSuffix))) {
            currentSuffix++
        }
        return this.getVolxitFullName(currentSuffix)
    }

    getVolxitFullName(suffix: number) {
        return `volxit-${suffix < 10 ? '0' : ''}${suffix}`
    }
