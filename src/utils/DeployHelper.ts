...
export default class DeployHelper {
    ...
    async startDeploy(deployParams: IDeployParams): Promise<boolean> {
        ...
        const machineToDeploy = deployParams.volxitMachine

        if (!appName || !machineToDeploy) {
            StdOutUtil.printError(
                "Can't deploy: missing VolxitCloud machine or app name.\n",
                true
            )
            return false
        }
        ...
        const tarFileNameToDeploy = tarFilePath
            ? tarFilePath
            : 'temporary-volxit-to-deploy.tar'
