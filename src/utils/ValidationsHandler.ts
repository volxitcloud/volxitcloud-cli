...
export function validateIsGitRepository() {
    ...
    if (!commandExistsSync('git')) {
        StdOutUtil.printError(
            '"git" command not found: VolxitCloud needs "git" to create tar file from your branch source files.\n' +
                'Run "volxitcloud deploy --help" to know more deployment options... (e.g. tar file or image name)\n',
            true
        )
    }
    ...
    if (!fs.pathExistsSync('./volxit-definition')) {
        if (fs.pathExistsSync('./Dockerfile')) {
            ...
            StdOutUtil.printMessage(
                'No volxit-definition was found in main directory: falling back to Dockerfile.\n'
            )
        } else {
            StdOutUtil.printWarning('**** Warning ****')
            StdOutUtil.printMessage(
                'No volxit-definition was found in main directory: unless you have specified a special path for your volxit-definition, this build will fail!\n'
            )
        }
    } else {
        let content = null
        try {
            content = JSON.parse(
                fs.readFileSync('./volxit-definition', 'utf8')
            )
        } catch (e) {
            StdOutUtil.printError(
                `volxit-definition file is not a valid JSON!\n${e.message ||
                    e}\n`,
                true
            )
        }
        if (!content || !content.schemaVersion) {
            StdOutUtil.printError(
                'volxit-definition needs "schemaVersion": please see docs!\n',
                true
            )
        }
    }
...
export function getErrorForAppName(
    ...
    if (!app) {
        return `${StdOutUtil.getColoredAppName(
            value
        )} app not exist on this VolxitCloud machine.`
    }
